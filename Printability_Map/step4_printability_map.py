...
from tensorflow.keras.models import load_model # Claude added Tensorflow intergrated keras
...
def ROM_THERMO(results_df):
    ...
    for i in range(results_df.shape[0]):
        if i%10 == 0 or i == total_count - 1:
            print(f"Processing Composition : {i + 1} / {total_count} ({(i+1)/total_count*100:.1f}%)")
...
def ET_NN(dimensionless_df):
    ...
    def load_pipeline_keras(model, folder_name="model"):
        ...
        build_model = lambda: load_model(folder_name+'/'+model, compile=False)
...
if __name__ == '__main__':
    ...
    # --- 1. Slurm Job Array Configuration ---
    # Fetch the ID assigned by Slurm to this specific sub-job
    batch_idx = int(os.environ.get('SLURM_ARRAY_TASK_ID', 0))
    total_batches = 10 # We split 6006 compositions into 10 batches for memory safety

    # --- 2. Data Loading and Batch Slicing ---
    file_name = 'RHEA'
    # Read the full datasets once
    thermo_calc_df_full = pd.read_csv('THERMOCALC_RHEA.csv')
    composition_df_full = pd.read_csv(file_name + '.csv')

    # Calculate row indices for this specific batch
    rows_per_batch = len(composition_df_full) // total_batches
    start_row = batch_idx * rows_per_batch
    # Ensure the final batch captures any remainder rows
    end_row = (batch_idx + 1) * rows_per_batch if batch_idx < total_batches - 1 else len(composition_df_full)

    # Slice the data: This ensures each node only processes its 1/10th share
    ...
    print(f"Batch {batch_idx} Starting: Processing Rows {start_row} to {end_row}")
    print(f"Current Batch Composition Count: {len(composition_df)} rows")

    # --- 3. Model Selection ---
    # Choose the E-T model type: 'analytical', 'NN', or 'scaled'
    e_t_model_type = 'NN'
    PATH = os.getcwd()

    # --- 4. Processing Parameter Definitions ---
    # Power range: 100W to 500W in 1W steps (400 points)
    power_w = list(np.arange(100, 500, 1))
    
    # Velocity range: 0.05m/s to 2.00m/s in 0.01m/s steps (196 points)
    vel_ms = list(np.arange(0.05, 2.01, 0.01))

    # Constant Manufacturing Parameters
    powder_thickness = [40]       # um
    hatch_spacing = [80]          # um
    d_laser = [80]                # um (laser diameter)
    powder_grain_size = [20]      # um
    laser_wavelength_nm = [1070]  # nm (standard fiber laser)
    T_amb = [288]                 # K (ambient temperature)

    # Criteria threshold values
    dim_key_value = [1.8]         # Keyholing criteria threshold
    dim_ball_value = [2.0]        # Balling criteria threshold
...
# Define the final criteria set 1 (1,3,6)
    ...
    df_combined.to_csv(f'Package_output_{file_name}_{e_t_model_type}_batch_{batch_idx}.csv') 
    results_df.to_csv(f'Material_PROP_{file_name}_{e_t_model_type}_batch_{batch_idx}.csv')
